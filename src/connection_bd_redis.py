...
class Bd_Redis:
    ...
    def calculate_sha256(self,file_content):
        """Calcule le hash (identifiant unique au contenu d'un doc) du fichier file

        Args:
            file_content (str): contenu du fichier (lu en 'rb')
    ...
    def get_content_from_key(self, key):
        """retourne le contenu du document de clé 'key' en markdown
    ...
    def add_value(self,key:str,value:str,hash=None,file_content=None):
        ...
        Args:
            ...
            file_content (str, optional): si le hash vaut None, le code doit calculer le hash pour cela, file prend le contenu du fichier (lu en 'rb'). Defaults to None.
        """
        if not(hash):
            value_add = {"content":value,"hash":str(self.calculate_sha256(file_content))}
        else:
    ...
    def add_from_corpus_txt(self,corpus_txt,origin_path,no_maj=False):
        ...
        Args:
            ...
            ATTENTION Si vous changez de source de données ou changer une partie des données (maj) il faut absolument que no_maj=False
        """
...
async def process_one_doc_trough_docling(chemin_doc,return_file_name_and_path=False):
    ...
    Rq : Méthode lente -> Pose problème sur des gros jeux de données
    Args:
        chemin_doc (str): chemin du fichier que l'on souhaite transformé (avec des /)
    ...
    Returns:
        ...
        Si return_file_name_and_path == True
            Tuple(str,str) : (chemin_du_doc, contenu en markdown)
    """
    try :
...
async def process_x_doc_trough_docling(list_chemin_doc):
    """Envoie x documents à doclig-serve pour en récupérer le contenu en markdown

    Args:
        list_chemin_doc (List[str]): chemin du fichier que l'on souhaite transformé (avec des /)

    Returns:
        list[(str,str)]: liste de tuple de format (chemin_du_doc, contenu en markdown)
    """
...
async def add_db_from_repertory(db,repertory_path,parallel_docling_doc=100):
    """Ajoute un repertoire de documents de touts types (ex : pdf) (réccursivement) à une base de donnée 
    ...
    Args:
        db (<Bd_Redis>): base de donnée dans laquelle on souhaite ajouter le contenu
        repertory_path (str): chemin du dossier qu'on souhaite ajouter à la db
        parallel_docling_doc (int, optional): nombre de documents à envoyer en parallèle à docling. Defaults to 100.
    """
...
def update_data_base(db,dossier,vector_graphe_storage,maj_db=True,maj_data_rag=True):
    ...
    Args:
        db (<Bd_Redis>): objet de la classe bd_Redis. Base de donnée qu'on souhaite mettre à jour
    ...
    Returns:
        list[str] : liste des noms des fichiers txt qui ont été mise à jour ou ajouter à la base de donnée (ou qui doivent l'être selon la valeur de maj_db)
    """
    ...
    for nomfichier in os.scandir(dossier):
            ...
            if nomfichier.is_dir():#si le fichier est un dossier on re-rappelle la fonction sur ce dossier
                ...
                nom_txt = chemin_fich.replace("/","-")+".txt"
                found = False
                for key in db_scan_iter: # on parcours les clés de la base de données pour voir si il y en a une identique au nom du fichier
                    if nom_txt==key.decode("utf-8"):
                        found=True

                        #on regarde si le fichier n'a pas été modifié
                        ...
                        with open(dossier+"/"+nomfichier.name,'rb') as f:
                            ...
                            time.sleep(10)
                            if hash_db != hachage: #si le hashage a changé
                ...
                if not(found): #si le nom du document est nouveau
                    try : 
