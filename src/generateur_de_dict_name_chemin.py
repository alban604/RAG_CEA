...
def genere_dictionnaire(dossier_fichier):
    """Génère un dictionnaire d'équivalence entre les noms des fichiers (avec des tirets à la place des /) et les chemins des fichiers
...
def genere_json(dossier_fichier,vector_graph_storage):
    ...
    Le json généré écrasera l version précédente si existante
    Args:
        dossier_fichier (str): Dossier qui contient tout les fichiers (pdf,docs,...) à partir desquels on veut faire le dictionnaire (exemple : '/stic')
...
def ajoute_json(dictionnaire,vector_graph_storage):
    """Ajoute à un json existant un ensemble de set de valeur (nom,chemin) présent dans le dictionnairee "dictionnaire" dans le json "from_name_to_chemin.json"
    Cela permet de mettre à jour le système de récupération des sources sans avoir à recalculer tout le dictionnaire json

    Args:
        dictionnaire (Dict[str:str]): dictionnaire qui associe à un nom txt (ex : chemin-to-doc.pdf.txt) le chemin du fichier (ex: chemin/to/doc.pdf) 
    ...
    with open(vector_graph_storage+"/from_name_to_chemin.json",'w') as f:  
        for key in dictionnaire.keys():
            dictio_json[key]=dictionnaire[key]
